...
export default function PortalKaryawan({onLogout}:{onLogout?:()=>void}){
 ...
 return <div className="employee-portal"><header className="employee-topbar"><div className="employee-brand"><div className="employee-logo">M</div><div><strong>MoonXprojecT</strong><small>Employee Self Service</small></div></div><div className="employee-user"><div className="notification-dot">{stats.unread}</div><div><b>{employee.nama}</b><small>{employee.jabatan||'Karyawan'}</small></div><button className="portal-logout" onClick={logout}>Keluar</button></div></header>
