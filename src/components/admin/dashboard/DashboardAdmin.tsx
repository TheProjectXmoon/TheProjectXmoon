...
function Icon({ name }: { name: string }) {
  const paths: Record<string, string> = {
    ...
    settings: 'M12 8a4 4 0 1 0 0 8 4 4 0 0 0 0-8m0-6v3m0 14v3m10-10h-3M5 12H2m17.1-7.1-2.1 2.1M7 17l-2.1 2.1m12.2 0L15 17M7 7 4.9 4.9',
    bell: 'M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9m-8 13h6',
    health: 'M20 12h-4l-2 7-4-14-2 7H4',
    card: 'M5 4h14v16H5zM8 8h8M8 12h5M8 16h8',
...
export default function DashboardAdmin() {
  ...
  // 1. DEFINISIKAN menuGroups TERLEBIH DAHULU DI ATAS (Agar bisa diakses useEffect & helper)
  const menuGroups = [
    {
      title: 'UTAMA',
      items: [
  ...
  useEffect(() => {
    ...
    const loadSession = async () => {
      ...
      if (!isSupabaseConfigured) {
        setError('Supabase belum dikonfigurasi. Isi VITE_SUPABASE_URL dan VITE_SUPABASE_ANON_KEY pada environment deployment.');
      ...
      if (data.user?.email) {
        ...
        if (active && p?.status === 'Aktif') {
          ...
          if (active) {
            setDbPerms((rp || []).map((x: any) => x.permission_code));
  ...
  useEffect(() => {
    const read = () => {
      ...
      if (candidate && menuGroups.flatMap((g: any) => g.items).some((x: any) => x[0] === candidate) && menuPermissionForRole(candidate, userRole, dbPerms)) {
        setMenu(candidate);
      }
    };
    read();
  ...
  async function login(e: FormEvent) {
    ...
    if (!profile || profile.status !== 'Aktif') {
      await signOut();
      setError('Akun tidak memiliki akses Dashboard HR.');
      return;
    }
  ...
  async function removeEmployee(k: Karyawan) {
    if (!menuPermissionForRole('employees', userRole, dbPerms) || !canDelete(dbPerms, 'people', userRole)) {
      setError('Anda tidak memiliki permission people.delete.');
      return;
    }
  ...
  const activeLabel = menuGroups.flatMap((g: any) => g.items).find((x: any) => x[0] === menu)?.[1] || 'Overview';
  ...
  if (!logged) return <Login email={email} pin={pin} setEmail={setEmail} setPin={setEmail} onSubmit={login} loading={loading} error={error} />;
   ...
   <div className="login-wrap">
     <div className="login-card">
       <div className="brand center">
         ...
         <div><b>MoonXprojecT</b><small>People Platform</small></div>
       </div>
       <h1>Selamat datang kembali</h1>
       <p>Masuk ke dashboard HR & payroll.</p>
       <form onSubmit={p.onSubmit}>
         <label>Email<input value={p.email} onChange={e => p.setEmail(e.target.value)} required/></label>
         <label>PIN / Password<input type="password" value={p.pin} onChange={e => p.setPin(e.target.value)} required/></label>
       ...
       <small className="security-note">Gunakan email dan password Supabase Auth yang diberikan HR.</small>
     </div>
   </div>
 );
}
...
function Stat({ title, value, hint, icon }: { title: string; value: string; hint: string; icon: string }) {
 return <div className="stat-card"><div className="stat-icon"><Icon name={icon}/></div><div><span>{title}</span><strong>{value}</strong><small>{hint}</small></div></div>;
}

function Quick({ label, icon, onClick }: { label: string; icon: string; onClick: () => void }) {
 return <button className="quick-action" onClick={onClick}><span className="quick-icon"><Icon name={icon}/></span>{label}<span aria-hidden="true">›</span></button>;
}
...
function Employees({ data, onDelete, onEdit, onExport, onAdd, onConfirmEmail }: { data: Karyawan[]; onDelete: (k: Karyawan) => void; onEdit: (k: Karyawan) => void; onExport: () => void; onAdd: () => void; onConfirmEmail: (k: Karyawan) => void }) {
 return (
  <>
   <Heading title="Semua Karyawan" desc="Master data workforce yang tersimpan di Supabase." action="Tambah Karyawan" onAction={onAdd}/>
   <div className="toolbar"><b>{data.length} karyawan</b><button className="secondary" onClick={onExport}>Export CSV</button></div>
   <div className="panel table-panel">
    <div className="table-wrap">
     <table>
      <thead><tr><th>Nama</th><th>ID</th><th>Jabatan</th><th>Departemen</th><th>Status</th><th>Gaji Pokok</th><th>Aksi</th></tr></thead>
      <tbody>{data.length ? data.map(k => <tr key={k.id}><td><div className="person"><div className="mini-avatar">{k.nama?.[0] || 'K'}</div><b>{k.nama}</b></div></td><td>{k.id_karyawan || '-'}</td><td>{k.jabatan || '-'}</td><td>{k.departemen || '-'}</td><td><Status value={k.status_aktif === false ? 'Nonaktif' : 'Aktif'}/></td><td>{money(Number(k.gaji_pokok || 0))}</td><td>
       <div className="row-actions">
         ...
         {k.email && (
           <button
             ...
             title={k.email_terverifikasi ? 'Email sudah terverifikasi' : !k.auth_user_id ? 'Akun login belum terhubung' : 'Konfirmasi email'}
           >
             {k.email_terverifikasi ? '✓ Email Terverifikasi' : !k.auth_user_id ? 'Akun Belum Terhubung' : 'Konfirmasi Email'}
           </button>
         )}
         <button className="danger-text" onClick={() => onDelete(k)}>Hapus</button>
       </div>
      </td></tr>) : <Empty cols={7}/>}</tbody>
     </table>
    </div>
   </div>
  </>
...
function AddEmployee({ onDone, refresh }: { onDone: () => void; refresh: () => void }) {
 const [f, setF] = useState({ id_karyawan: '', nama: '', jabatan: '', email: '', no_telp: '', departemen: '', tanggal_masuk: '', gaji_pokok: '' });
 ...
 return (
  <>
   <Heading title="Tambah Karyawan" desc="Simpan profil baru langsung ke tabel karyawan."/>
   <div className="panel form-panel">
    <form className="form-grid" onSubmit={save}>
     {Object.entries(f).map(([k, v]) => <label key={k}>{fieldLabel(k)}<input required={['id_karyawan', 'nama'].includes(k)} type={k === 'gaji_pokok' ? 'number' : k === 'tanggal_masuk' ? 'date' : 'text'} value={String(v ?? '')} onChange={e => setF({ ...f, [k]: e.target.value })}/></label>)}
     {msg && <div className="form-error full-span">{msg}</div>}
     <div className="full-span form-actions"><button type="button" className="secondary" onClick={onDone}>Batal</button><button className="primary" disabled={saving}>{saving ? 'Menyimpan…' : 'Simpan Karyawan'}</button></div>
    </form>
   </div>
  </>
 );
 ...
 return (
  <div className="drawer-backdrop" onMouseDown={e => { if (e.currentTarget === e.target) onClose(); }}>
   <aside className="edit-drawer">
    <div className="drawer-head"><div><span>EMPLOYEE PROFILE</span><h2>Edit Karyawan</h2></div><button className="icon-btn" onClick={onClose}>×</button></div>
    <div className="drawer-body">{Object.entries(f).filter(([k]) => k !== 'status_aktif').map(([k, v]) => <label key={k}>{fieldLabel(k)}<input type={k === 'gaji_pokok' ? 'number' : k === 'tanggal_masuk' ? 'date' : 'text'} value={String(v ?? '')} onChange={e => setF({ ...f, [k]: e.target.value })}/></label>)}<label className="switch-row"><span>Status Aktif</span><input type="checkbox" checked={f.status_aktif} onChange={e => setF({ ...f, status_aktif: e.target.checked })}/></label></div>
    <div className="drawer-foot"><button className="secondary" onClick={onClose}>Batal</button><button className="primary" onClick={() => onSave({ ...f, gaji_pokok: Number(f.gaji_pokok || 0) })}>Simpan Perubahan</button></div>
   </aside>
  </div>
 );
}

function Branch({ title, desc, items, tab, setTab, action, onAction, children }: { title: string; desc: string; items: { key: string; label: string; icon: string }[]; tab: string; setTab: (v: string) => void; action?: string; onAction?: () => void; children: ReactNode }) {
 return (
  <>
   <Heading title={title} desc={desc} action={action} onAction={onAction}/>
   <div className="branch-nav">{items.map(i => <button key={i.key} className={tab === i.key ? 'active' : ''} onClick={() => setTab(i.key)}><span>{i.icon}</span>{i.label}</button>)}</div>
   {children}
  </>
 );
}
...
function AttendanceModule({ type, data, onRefresh, onExport }: { type: MenuKey; data: Absensi[]; onRefresh: () => void; onExport: () => void }) {
 ...
 return (
  <Branch title="Absensi" desc="Rekap, input manual, review keterlambatan, lembur, dan selfie." items={items.map(([key, label, icon]) => ({ key, label, icon }))} tab={tab} setTab={setTab} action={tab === 'summary' ? '＋ Input Absensi' : 'Export CSV'} onAction={tab === 'summary' ? () => setOpen(true) : onExport}>
...
function HolidayModule() {
 ...
 const [f, setF] = useState({ tanggal: isoToday(), nama: '', tipe: 'Nasional' });
 const [msg, setMsg] = useState('');
 const load = async () => { const { data, error } = await supabase.from('hris_hari_libur').select('*').order('tanggal'); if (error) setMsg(error.message); else setRows(data || []); };
 useEffect(() => { load(); }, []);
 ...
 return (
  <>
   <Heading title="Hari Libur" desc="Kelola tanggal non-working day di database." action="＋ Tambah Hari Libur" onAction={() => setModal(true)}/>
   {msg && <div className="alert">{msg}</div>}
   <div className="panel table-panel"><div className="table-wrap"><table><thead><tr><th>Tanggal</th><th>Nama</th><th>Tipe</th><th>Aksi</th></tr></thead><tbody>{rows.length ? rows.map(r => <tr key={r.id}><td>{r.tanggal}</td><td><b>{r.nama}</b></td><td><Status value={r.tipe}/></td><td><button className="danger-text" onClick={() => del(r.id)}>Hapus</button></td></tr>) : <Empty cols={4}/>}</tbody></table></div></div>
   {modal && <SimpleModal title="Tambah Hari Libur" onClose={() => setModal(false)} onSave={save}><label>Tanggal<input type="date" value={f.tanggal} onChange={e => setF({ ...f, tanggal: e.target.value })}/></label><label>Nama Hari Libur<input required value={f.nama} onChange={e => setF({ ...f, nama: e.target.value })}/></label><label>Tipe<select value={f.tipe} onChange={e => setF({ ...f, tipe: e.target.value })}><option>Nasional</option><option>Perusahaan</option></select></label></SimpleModal>}
  </>
 );
}

function LeaveModule({ initial }: { initial: MenuKey }) {
 const [tab, setTab] = useState(initial === 'leave-balance' ? 'balance' : 'requests');
 const [rows, setRows] = useState<any[]>([]);
 ...
 return (
  <Branch title="Cuti" desc="Pengajuan, approval, dan saldo cuti tersimpan di database." items={items} tab={tab} setTab={setTab} action={tab === 'requests' ? '＋ Buat Pengajuan' : undefined} onAction={() => setModal(true)}>
   {tab === 'balance' ? <div className="panel table-panel"><div className="table-wrap"><table><thead><tr><th>Karyawan</th><th>Jenis</th><th>Jatah</th><th>Terpakai</th><th>Sisa</th><th>Aksi</th></tr></thead><tbody>{employees.map(k => { const b = balances.find(x => x.id_karyawan === k.id_karyawan); const quota = Number((b?.saldo ?? 12)) + Number((b?.terpakai ?? 0)); const used = Number((b?.terpakai ?? 0)); return <tr key={k.id}><td><b>{k.nama}</b><small>{k.id_karyawan}</small></td><td>Tahunan</td><td>{quota}</td><td>{used}</td><td><Status value={String(Math.max(0, quota - used))}/></td><td><button className="link-btn" onClick={() => k.id_karyawan && ensureBalance(k.id_karyawan).then(load)}>Inisialisasi</button></td></tr>; })}</tbody></table></div></div> : <div className="panel table-panel"><div className="table-wrap"><table><thead><tr><th>Karyawan</th><th>Jenis</th><th>Tanggal</th><th>Alasan</th><th>Status</th><th>Aksi</th></tr></thead><tbody>{rows.length ? rows.map(r => <tr key={r.id}><td>{r.id_karyawan}</td><td>{r.jenis}</td><td>{r.tanggal_mulai} s/d {r.tanggal_selesai}</td><td>{r.alasan || '-'}</td><td><Status value={r.status}/></td><td>{r.status === 'Menunggu' && <><button className="link-btn" onClick={() => update(r.id, 'Disetujui')}>Setujui</button> <button className="danger-text" onClick={() => update(r.id, 'Ditolak')}>Tolak</button></>}</td></tr>) : <Empty cols={6}/>}</tbody></table></div></div>}
   {modal && <SimpleModal title="Pengajuan Cuti" onClose={() => setModal(false)} onSave={save}><label>Karyawan<select required value={f.id_karyawan} onChange={e => setF({ ...f, id_karyawan: e.target.value })}><option value="">Pilih karyawan</option>{employees.map(k => <option key={k.id_karyawan} value={k.id_karyawan}>{k.nama} — {k.id_karyawan}</option>)}</select></label><label>Jenis<select value={f.jenis} onChange={e => setF({ ...f, jenis: e.target.value })}><option>Tahunan</option><option>Sakit</option><option>Khusus</option></select></label><label>Mulai<input type="date" value={f.tanggal_mulai} onChange={e => setF({ ...f, tanggal_mulai: e.target.value })}/></label><label>Selesai<input type="date" value={f.tanggal_selesai} onChange={e => setF({ ...f, tanggal_selesai: e.target.value })}/></label><label>Jumlah Hari<input type="number" min="0.5" step="0.5" value={f.jumlah_hari} onChange={e => setF({ ...f, jumlah_hari: e.target.value })}/></label><label>Alasan<textarea value={f.alasan} onChange={e => setF({ ...f, alasan: e.target.value })}/></label></SimpleModal>}
  </Branch>
 );
}

function TalentModule({ initial, employees }: { initial: MenuKey; employees: Karyawan[] }) {
 const [tab, setTab] = useState(initial === 'kpi' ? 'kpi' : initial === 'recruitment' ? 'vacancies' : initial === 'candidates' ? 'candidates' : 'performance');
 const [rows, setRows] = useState<any[]>([]);
 const [modal, setModal] = useState(false);
 const load = async () => { const table = tab === 'kpi' ? 'hris_kpi' : tab === 'vacancies' ? 'hris_lowongan' : tab === 'candidates' ? 'hris_kandidat' : tab === 'interviews' ? 'hris_interview' : 'hris_performance'; const { data, error } = await supabase.from(table).select('*').order('created_at', { ascending: false }); if (!error) setRows(data || []); else setRows([]); };
 useEffect(() => { load(); }, [tab]);
...
function TalentTable({ tab, rows }: { tab: string; rows: any[] }) {
 let cols: string[] = [];
 if (tab === 'kpi') cols = ['id_karyawan', 'periode', 'indikator', 'target', 'realisasi', 'skor', 'status'];
 else if (tab === 'vacancies') cols = ['posisi', 'departemen', 'jumlah_kebutuhan', 'status', 'tanggal_buka', 'tanggal_tutup'];
 else if (tab === 'candidates') cols = ['nama', 'email', 'no_telp', 'posisi', 'tahap', 'status'];
 else if (tab === 'interviews') cols = ['kandidat', 'tanggal', 'jam', 'interviewer', 'hasil', 'status'];
 else cols = ['id_karyawan', 'periode', 'nilai', 'catatan', 'status'];
 return <div className="panel table-panel"><div className="table-wrap"><table><thead><tr>{cols.map(c => <th key={c}>{fieldLabel(c)}</th>)}</tr></thead><tbody>{rows.length ? rows.map(r => <tr key={r.id}>{cols.map(c => <td key={c}>{c === 'status' ? <Status value={String(r[c] ?? '-')}/> : String(r[c] ?? '-')}</td>)}</tr>) : <Empty cols={cols.length}/>}</tbody></table></div></div>;
}
...
function Reports({ employees, attendance, onExport }: { employees: Karyawan[]; attendance: Absensi[]; onExport: (r: any[], f: string) => void }) {
 ...
 useEffect(() => { if (tab === 'payroll') supabase.from('hris_payroll').select('*').order('created_at', { ascending: false }).limit(2000).then(({ data }) => setPayroll(data || [])); }, [tab]);
 const items = [['overview', 'Analytics', 'report'], ['attendance', 'Laporan Absensi', 'clock'], ['payroll', 'Laporan Payroll', 'payroll'], ['people', 'Laporan Karyawan', 'users']].map(([key, label, icon]) => ({ key, label, icon }));
 return <Branch title="Laporan" desc="Export data nyata dari database." items={items} tab={tab} setTab={setTab}>{tab === 'overview' ? <div className="report-grid"><ReportCard name="Master Karyawan" count={employees.length} onClick={() => onExport(employees, 'laporan-karyawan.csv')}/><ReportCard name="Absensi" count={attendance.length} onClick={() => onExport(attendance, 'laporan-absensi.csv')}/><ReportCard name="Payroll" count={payroll.length} onClick={() => onExport(payroll, 'laporan-payroll.csv')}/></div> : tab === 'attendance' ? <ReportCard name="Laporan Absensi" count={attendance.length} onClick={() => onExport(attendance, 'laporan-absensi.csv')}/> : tab === 'people' ? <ReportCard name="Laporan Karyawan" count={employees.length} onClick={() => onExport(employees, 'laporan-karyawan.csv')}/> : <ReportCard name="Laporan Payroll" count={payroll.length} onClick={() => onExport(payroll, 'laporan-payroll.csv')}/>}</Branch>;
}

function ReportCard({ name, count, onClick }: { name: string; count: number; onClick: () => void }) {
 return <div className="report-card"><span>REPORT</span><h3>{name}</h3><b>{count}</b><p>record tersedia</p><button className="primary" onClick={onClick}>Export CSV</button></div>;
}

function Settings() {
 const [f, setF] = useState<any>({ company_name: 'MoonXprojecT', work_start: '07:00', work_end: '16:00', break_minutes: 60, payday_day: 'Jumat', currency: 'IDR', timezone: 'Asia/Jakarta', overtime_multiplier: 2, late_tolerance_minutes: 10, attendance_radius_meters: 100, attendance_latitude: '', attendance_longitude: '', attendance_max_accuracy_meters: 100, auto_approve_attendance: false, notify_late: true, notify_leave: true, maintenance_mode: false });
 const [tab, setTab] = useState('Perusahaan');
 const [msg, setMsg] = useState('');
 const [customTheme, setCustomTheme] = useState({ primary: '#101a33', accent: '#d6ae58', background: '#f6f7fb', surface: '#ffffff', text: '#172033', border: '#d6ae58' });
 const themes = [
  { id: 'moon', name: 'MoonXprojecT — Navy Gold', description: 'Tema utama enterprise MoonXprojecT.', primary: '#101a33', accent: '#d6ae58', background: '#f6f7fb', surface: '#ffffff', text: '#172033', border: '#d6ae58' },
 ...
 const applyTheme = (theme: any) => {
  ...
  const primary = theme.primary || '#101a33';
  const accent = theme.accent || '#d6ae58';
  const background = theme.background || '#f6f7fb';
  const surface = theme.surface || '#ffffff';
  const text = theme.text || '#172033';
  const border = theme.border || accent;
  root.style.setProperty('--mx-primary', primary);
  root.style.setProperty('--mx-accent', accent);
  root.style.setProperty('--mx-background', background);
  ...
  root.style.setProperty('--blue2', primary);
  root.style.setProperty('--blue-soft', background);
  root.style.setProperty('--ink', text);
  root.style.setProperty('--line', border);
  root.style.setProperty('--surface', surface);
  root.style.setProperty('--bg', background);
  setCustomTheme({ primary, accent, background, surface, text, border });
  localStorage.setItem('moonx-theme', JSON.stringify({ ...theme, primary, accent, background, surface, text, border }));
  setMsg(`Tema "${theme.name || 'Custom Theme'}" berhasil diterapkan.`);
 };

 const updateCustomColor = (key: keyof typeof customTheme, value: string) => {
  setCustomTheme(prev => ({ ...prev, [key]: value }));
  const root = document.documentElement;
  const map: Record<keyof typeof customTheme, string> = { primary: '--mx-primary', accent: '--mx-accent', background: '--mx-background', surface: '--mx-surface', text: '--mx-text', border: '--mx-border' };
  root.style.setProperty(map[key], value);
  if (key === 'primary') root.style.setProperty('--blue2', value);
  if (key === 'accent') root.style.setProperty('--blue', value);
  if (key === 'background') root.style.setProperty('--bg', value);
  if (key === 'surface') root.style.setProperty('--surface', value);
  if (key === 'text') root.style.setProperty('--ink', value);
  if (key === 'border') root.style.setProperty('--line', value);
 };
 ...
 useEffect(() => {
  ...
  if (saved) { try { applyTheme(JSON.parse(saved)); } catch { applyTheme(themes[0]); } } else { applyTheme(themes[0]); }
 }, []);
 ...
 const saveCustomTheme = () => {
  const theme = { id: 'custom', name: 'Custom Theme', description: 'Tema kustom MoonXprojecT', ...customTheme };
  const root = document.documentElement;
  ...
  root.style.setProperty('--blue2', customTheme.primary);
  root.style.setProperty('--blue-soft', customTheme.background);
  ...
  setMsg('Custom Theme berhasil disimpan dan diterapkan.');
 };
 ...
 const groups: any = {
  ...
  'Absensi': ['attendance_radius_meters', 'attendance_latitude', 'attendance_longitude', 'attendance_max_accuracy_meters', 'auto_approve_attendance'],
  'Notifikasi': ['notify_late', 'notify_leave'],
  'Keamanan': ['maintenance_mode']
 };

 const labels: any = {
  company_name: 'Nama Perusahaan', currency: 'Mata Uang', timezone: 'Zona Waktu', work_start: 'Jam Masuk', work_end: 'Jam Pulang', break_minutes: 'Istirahat (menit)', late_tolerance_minutes: 'Toleransi Terlambat (menit)', payday_day: 'Hari Gajian', overtime_multiplier: 'Pengali Lembur', attendance_radius_meters: 'Radius Absensi (meter)', attendance_latitude: 'Latitude Lokasi Absensi', attendance_longitude: 'Longitude Lokasi Absensi', attendance_max_accuracy_meters: 'Maksimal Akurasi GPS (meter)', auto_approve_attendance: 'Auto Approve Absensi', notify_late: 'Notifikasi Keterlambatan', notify_leave: 'Notifikasi Cuti', maintenance_mode: 'Mode Maintenance'
 };

 return (
  <>
   <Heading title="Pengaturan" desc="Kelola konfigurasi perusahaan, operasional, payroll, absensi, keamanan, dan tampilan sistem." action="Simpan Perubahan" onAction={save}/>
   {msg && <div className="theme-message">{msg}</div>}
   <div className="settings-tabs">
    {Object.keys(groups).map(x => <button key={x} type="button" className={tab === x ? 'active' : ''} onClick={() => setTab(x)}>{x}</button>)}
    <button type="button" className={tab === 'Tampilan & Tema' ? 'active' : ''} onClick={() => setTab('Tampilan & Tema')}>🎨 Tampilan & Tema</button>
   </div>
   {tab === 'Tampilan & Tema' ? (
    <div className="theme-manager">
     <div className="theme-manager-header"><div><h2>Tampilan & Tema</h2><p>Pilih tampilan visual yang digunakan oleh HRIS MoonXprojecT.</p></div></div>
     <div className="theme-grid">
      {themes.map(theme => (
       <button type="button" key={theme.id} className="theme-card" onClick={() => applyTheme(theme)}>
        <div className="theme-preview" style={{ background: theme.background }}>
         <div className="theme-preview-sidebar" style={{ background: theme.primary }}><span/><span/><span/><span/></div>
         <div className="theme-preview-content">
          <div className="theme-preview-top" style={{ borderColor: theme.border }}></div>
          <div className="theme-preview-cards"><i style={{ background: theme.accent }}/><i style={{ background: theme.primary }}/><i style={{ background: theme.border }}/></div>
          <div className="theme-preview-line" style={{ background: theme.accent }}></div>
         </div>
        </div>
        <div className="theme-card-body">
         <div><strong>{theme.name}</strong><small>{theme.description}</small></div>
         <span className="theme-color-dot" style={{ background: theme.accent }}></span>
        </div>
       </button>
      ))}
     </div>
     <div className="custom-theme-panel">
      <div><h3>Custom Theme</h3><p>Gunakan warna pilihan Anda untuk tampilan HRIS.</p></div>
      <div className="custom-theme-controls">
       {(['primary', 'accent', 'background', 'surface', 'text', 'border'] as const).map(key => (
        <label key={key}>{key.charAt(0).toUpperCase() + key.slice(1)}<input type="color" value={customTheme[key]} onChange={e => updateCustomColor(key, e.target.value)}/></label>
       ))}
      </div>
      <div className="custom-theme-actions"><button type="button" className="primary theme-save-button" onClick={saveCustomTheme}>Simpan Custom Theme</button></div>
     </div>
    </div>
   ) : (
    <div className="panel form-panel settings-content">
     <div className="form-grid">
      {groups[tab].map((k: string) => {
       const v = f[k];
       const bool = ['auto_approve_attendance', 'notify_late', 'notify_leave', 'maintenance_mode'].includes(k);
       return (
        <label key={k}>
         {labels[k]}
         {bool ? (
          <input type="checkbox" checked={!!v} onChange={e => setF({ ...f, [k]: e.target.checked })}/>
         ) : (
          <input type={['break_minutes', 'late_tolerance_minutes', 'attendance_radius_meters', 'attendance_max_accuracy_meters', 'attendance_latitude', 'attendance_longitude'].includes(k) ? 'number' : k.includes('start') || k.includes('end') ? 'time' : 'text'} value={String(v ?? '')} onChange={e => setF({ ...f, [k]: ['break_minutes', 'late_tolerance_minutes', 'attendance_radius_meters', 'attendance_max_accuracy_meters', 'attendance_latitude', 'attendance_longitude'].includes(k) ? Number(e.target.value) : e.target.value })}/>
         )}
        </label>
       );
      })}
...
function Audit() {
 ...
 useEffect(() => {
  ...
  const loadAudit = async () => {
   setLoading(true);
   const { data, error } = await supabase.from('hris_audit_logs').select('*').order('created_at', { ascending: false }).limit(500);
   if (mounted) { setRows(error ? [] : data || []); setLoading(false); }
  };
  loadAudit();
 ...
 const getChangedFields = (row: any) => {
  const oldValue = row?.old ?? row?.old_data ?? row?.old_values ?? row?.metadata?.old ?? {};
  const newValue = row?.new ?? row?.new_data ?? row?.new_values ?? row?.metadata?.new ?? {};
  const oldObj = oldValue && typeof oldValue === 'object' ? oldValue : {};
  const newObj = newValue && typeof newValue === 'object' ? newValue : {};
  const keys = Array.from(new Set([...Object.keys(oldObj), ...Object.keys(newObj)]));
  return keys.filter(key => JSON.stringify(oldObj[key]) !== JSON.stringify(newObj[key])).map(key => ({ field: key, oldValue: oldObj[key], newValue: newObj[key] }));
 };

 const formatDate = (value: any) => {
  if (!value) return '-';
  try { return new Date(value).toLocaleString('id-ID', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' }); } catch { return String(value); }
 };
 ...
 const filteredRows = rows.filter(row => {
  ...
  const searchable = [row.actor_email, row.actor_name, row.action, row.module, row.description, row.entity_id, row.entity_type, JSON.stringify(row.details || {}), JSON.stringify(row.old || {}), JSON.stringify(row.new || {})].filter(Boolean).join(' ').toLowerCase();
 ...
 const modules = Array.from(new Set(rows.map(row => String(row.module || '-')).filter(Boolean)));

 return (
  <>
   <Heading title="Audit Log" desc="Riwayat aktivitas dan perubahan data yang tercatat di database."/>
   <div className="panel" style={{ marginBottom: 16 }}>
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(220px, 1fr) 180px 180px auto', gap: 10, alignItems: 'center' }}>
     <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Cari actor, action, module, ID..." style={{ width: '100%', padding: '11px 13px', borderRadius: 10, border: '1px solid #d9dee8', outline: 'none' }}/>
     <select value={actionFilter} onChange={e => setActionFilter(e.target.value)} style={{ padding: '11px 13px', borderRadius: 10, border: '1px solid #d9dee8', background: '#fff' }}><option value="ALL">Semua Action</option><option value="CREATE">CREATE</option><option value="UPDATE">UPDATE</option><option value="DELETE">DELETE</option></select>
     <select value={moduleFilter} onChange={e => setModuleFilter(e.target.value)} style={{ padding: '11px 13px', borderRadius: 10, border: '1px solid #d9dee8', background: '#fff' }}><option value="ALL">Semua Module</option>{modules.map(module => <option key={module} value={module}>{module}</option>)}</select>
     <div style={{ fontSize: 13, color: '#667085', whiteSpace: 'nowrap' }}>{filteredRows.length} aktivitas</div>
    </div>
   </div>
   <div className="panel table-panel">
    <div className="table-wrap">
     <table>
      <thead><tr><th>Waktu</th><th>Actor</th><th>Action</th><th>Module</th><th>Entity</th><th>Perubahan</th><th style={{ textAlign: 'center' }}>Detail</th></tr></thead>
      <tbody>
       {loading ? <tr><td colSpan={7} style={{ textAlign: 'center', padding: 30 }}>Memuat Audit Log...</td></tr> : filteredRows.length ? filteredRows.map(row => {
        const changes = getChangedFields(row);
        return (
         <tr key={row.id}>
          <td style={{ whiteSpace: 'nowrap' }}>{formatDate(row.created_at)}</td>
          <td><div style={{ fontWeight: 600, color: '#101828' }}>{row.actor_email || row.actor_name || '-'}</div></td>
          <td><span className={actionClass(row.action)}>{actionLabel(row.action)}</span></td>
          <td>{row.module || '-'}</td>
          <td><div><strong>{row.entity_type || '-'}</strong></div>{row.entity_id && <small style={{ color: '#667085', wordBreak: 'break-all' }}>{row.entity_id}</small>}</td>
          <td>
           {changes.length ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
             {changes.slice(0, 3).map(change => (
              <div key={change.field} style={{ fontSize: 12 }}><strong>{change.field}</strong>: <span style={{ color: '#b42318' }}>{normalize(change.oldValue)}</span>{' → '}<span style={{ color: '#027a48' }}>{normalize(change.newValue)}</span></div>
             ))}
             {changes.length > 3 && <small style={{ color: '#667085' }}>+{changes.length - 3} perubahan lainnya</small>}
            </div>
           ) : <span style={{ color: '#98a2b3' }}>Tidak ada perubahan field</span>}
          </td>
          <td style={{ textAlign: 'center' }}><button type="button" onClick={() => setSelected(row)} style={{ border: '1px solid #d0d5dd', background: '#fff', borderRadius: 8, padding: '7px 11px', cursor: 'pointer', fontWeight: 600 }}>Detail</button></td>
         </tr>
        );
       }) : <Empty cols={7}/>}
      </tbody>
   ...
   {selected && (
    <div onClick={() => setSelected(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(15, 23, 42, 0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, zIndex: 9999 }}>
     <div onClick={e => e.stopPropagation()} style={{ width: 'min(1000px, 100%)', maxHeight: '85vh', overflow: 'auto', background: '#fff', borderRadius: 16, boxShadow: '0 20px 60px rgba(0,0,0,.2)', padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
       <div><h2 style={{ margin: 0, fontSize: 20 }}>Audit Detail</h2><div style={{ marginTop: 5, color: '#667085', fontSize: 13 }}>{formatDate(selected.created_at)}</div></div>
       <button type="button" onClick={() => setSelected(null)} style={{ border: 'none', background: '#f2f4f7', borderRadius: 8, padding: '8px 12px', cursor: 'pointer' }}>Tutup</button>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 12, marginBottom: 20 }}>
       <div><small>Actor</small><div style={{ fontWeight: 600 }}>{selected.actor_email || selected.actor_name || '-'}</div></div>
       <div><small>Action</small><div style={{ marginTop: 5 }}><span className={actionClass(selected.action)}>{actionLabel(selected.action)}</span></div></div>
       <div><small>Module</small><div style={{ fontWeight: 600 }}>{selected.module || '-'}</div></div>
       <div><small>Entity</small><div style={{ fontWeight: 600 }}>{selected.entity_type || '-'}</div></div>
      </div>
      <h3 style={{ margin: '0 0 12px', fontSize: 16 }}>Perubahan Data</h3>
      <div style={{ border: '1px solid #eaecf0', borderRadius: 12, overflow: 'hidden' }}>
       <table>
        ...
        <tbody>
         {getChangedFields(selected).length ? getChangedFields(selected).map(change => (
          <tr key={change.field}>
           <td><strong>{change.field}</strong></td>
           <td><span style={{ color: '#b42318', wordBreak: 'break-word' }}>{normalize(change.oldValue)}</span></td>
           <td><span style={{ color: '#027a48', wordBreak: 'break-word' }}>{normalize(change.newValue)}</span></td>
          </tr>
         )) : <tr><td colSpan={3} style={{ textAlign: 'center', padding: 20 }}>Tidak ada perubahan field.</td></tr>}
        </tbody>
       </table>
      </div>
      {selected.details && (
       <details style={{ marginTop: 20 }}>
        <summary style={{ cursor: 'pointer', fontWeight: 600 }}>Raw Details</summary>
        <pre style={{ marginTop: 10, background: '#101828', color: '#f8fafc', padding: 15, borderRadius: 10, overflow: 'auto', fontSize: 12 }}>{JSON.stringify(selected.details, null, 2)}</pre>
       </details>
      )}
     </div>
    </div>
...
function Notifications() {
 const [rows, setRows] = useState<any[]>([]);
 const [loading, setLoading] = useState(true);
 const load = async () => { setLoading(true); const { data } = await supabase.from('hris_notifications').select('*').order('created_at', { ascending: false }).limit(100); setRows(data || []); setLoading(false); };
 useEffect(() => { load(); }, []);
 const mark = async (id: string) => { await supabase.from('hris_notifications').update({ is_read: true }).eq('id', id); load(); };
 return <><Heading title="Notifikasi" desc="Pusat pemberitahuan HRIS untuk approval, cuti, payroll, dan aktivitas penting."/><div className="panel table-panel"><div className="panel-head"><div><h2>Inbox HR</h2><p>{rows.filter(r => !r.is_read).length} belum dibaca</p></div><button className="secondary" onClick={load}>Refresh</button></div><div className="notification-list">{loading ? <div className="loading">Memuat…</div> : rows.length ? rows.map(r => <button key={r.id} className={`notification-item ${r.is_read ? 'read' : ''}`} onClick={() => mark(r.id)}><span className="notification-dot"/><span><b>{r.title}</b><small>{r.message}</small><em>{r.created_at?.replace('T', ' ').slice(0, 19)}</em></span></button>) : <div className="empty-module"><h3>Tidak ada notifikasi</h3><p>Notifikasi sistem akan muncul di sini.</p></div>}</div></div></>;
}

function SystemHealth() {
 const [h, setH] = useState<any>(null);
 const [err, setErr] = useState('');
 const load = async () => { const { data, error } = await supabase.from('hris_system_health').select('*').maybeSingle(); if (error) setErr(error.message); else setH(data); };
 useEffect(() => { load(); }, []);
 const cards = [['active_employees', 'Karyawan Aktif'], ['pending_leave', 'Cuti Menunggu'], ['pending_overtime', 'Lembur Menunggu'], ['pending_payroll', 'Payroll Menunggu'], ['pending_approvals', 'Approval Menunggu'], ['unread_notifications', 'Notifikasi Belum Dibaca']];
 return <><Heading title="System Health" desc="Ringkasan kesehatan operasional HRIS dari database." action="Refresh" onAction={load}/>{err && <div className="alert">{err}</div>}<div className="mini-kpi-row">{cards.map(([k, l]) => <div className="stat-card" key={k}><span>{l}</span><strong>{h?.[k] ?? '—'}</strong></div>)}</div><div className="panel"><h3>Status layanan</h3><p>Database: <b>{h ? 'Operational' : 'Checking…'}</b></p><p>Terakhir diperiksa: {h?.checked_at?.replace('T', ' ').slice(0, 19) || '—'}</p></div></>;
}

function SimpleModal({ title, onClose, onSave, children }: { title: string; onClose: () => void; onSave: (e: FormEvent) => void; children: ReactNode }) {
 return <div className="drawer-backdrop"><aside className="edit-drawer"><div className="drawer-head"><h2>{title}</h2><button className="icon-btn" onClick={onClose}>×</button></div><form className="drawer-body" onSubmit={onSave}>{children}<div className="drawer-foot"><button type="button" className="secondary" onClick={onClose}>Batal</button><button className="primary">Simpan</button></div></form></aside></div>;
}

function Status({ value }: { value: string }) {
 const v = value.toLowerCase();
 const cls = v.includes('non') || v.includes('tolak') || v.includes('sakit') ? 'red' : v.includes('terlambat') || v.includes('draft') || v.includes('menunggu') ? 'orange' : v.includes('izin') ? 'blue' : 'green';
 return <span className={`status ${cls}`}>{value}</span>;
}

function Empty({ cols }: { cols: number }) {
 return <tr><td colSpan={cols} className="empty-cell">Belum ada data.</td></tr>;
}
